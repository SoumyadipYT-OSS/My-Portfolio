...
const techs = [
    ...
    { component: <SiJavascript className='rounded-sm text-5xl text-yellow-400 bg-white' />, bg: 'bg-yellow-400' },
    { component: <SiBlazor className='rounded-md text-5xl text-violet-800' />, bg: 'bg-slate-50' },
    { component: <SiTypescript className='text-5xl text-blue-500 bg-white' />, bg: 'bg-blue-500' },
...
const Technologies = () => {
    return (
        <div className="border-b border-neutral-800 pb24 mb-24">
            <h1 className="my-20 text-center text-2xl sm:text-3xl md:text-4xl">Technologies</h1>
            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-8 float-animation pb-10">
                {techs.map((tech, i) => (
                    <div key={i} className={`rounded-2xl border-4 border-neutral-700 p-2 sm:p-4 ${tech.bg}`} style={{ '--i': i }}>
                        {tech.logo ? (
                            <img src={tech.logo} alt={tech.alt} className={`h-8 w-8 sm:${tech.className}`} />
                        ) : (
                            <div className={`text-3xl sm:${tech.component.props.className}`}>{tech.component}</div>
                        )}
                    </div>
                ))}
            </div>
